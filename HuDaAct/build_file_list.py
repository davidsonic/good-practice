...
parser.add_argument('dataset', type=str, choices=['ucf101', 'hmdb51','gesture','huda'],default='huda')
parser.add_argument('frame_path', type=str, help="root directory holding the frames",default='./')
...
open(os.path.join(out_path, '{}_rgb_flow_train_split_{}.txt'.format(dataset, 1)), 'w').writelines(train_lists[1])
open(os.path.join(out_path, '{}_rgb_flow_val_split_{}.txt'.format(dataset, 1)), 'w').writelines(test_lists[1])
