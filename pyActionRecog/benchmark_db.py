...
def parse_directory(path, rgb_prefix='img_', flow_x_prefix='flow_x_', flow_y_prefix='flow_y_'):
    ...
    frame_folders = glob.glob(os.path.join(path, '*'))  #if HuDa *-depth
...
def build_gesture_train(split_tuple, frame_info, shuffle=False):
    split = split_tuple
    ...
    train_rgb_list, train_flow_list = build_set_list(split)
    return (train_rgb_list), (train_flow_list)


def build_gesture_test(split_tuple, frame_info, shuffle=False):
    split = split_tuple
    ...
    test_rgb_list, test_flow_list = build_set_list(split)
    return (test_rgb_list), (test_flow_list)
...
# train and test file from respectice files
def parse_gesture_splits():
    class_ind= [x.strip().split(' ')[1] for x in open('gesture/train_depth_list.txt')]
    class_ind=set(class_ind)
    class_mapping = {x:int(x)-1 for x in class_ind}

    def line2rec(line):
        items = line.strip().split(' ')
        label = class_mapping[items[1]]
        vid = items[0].split('/')[-1].split('.')[0]
        return vid, label

    splits = []
    train_list = [line2rec(x) for x in open('gesture/train_depth_list.txt')]
    test_list = [line2rec(x) for x in open('gesture/valid_depth.txt')]
    splits.append((train_list,test_list))
    return splits
...
# train and test file from respectice files
def parse_HuDa_splits():
    class_ind= [x.strip().split(' ')[1] for x in open('HuDaAct/train_with_label.txt')]
    class_ind=set(class_ind)
    class_mapping = {x:int(x) for x in class_ind}
    ...
    splits = []
    train_list = [line2rec(x) for x in open('HuDaAct/train_depth_label.txt')]
    test_list = [line2rec(x) for x in open('HuDaAct/test_depth_label.txt')]
    splits.append((train_list,test_list))
    return splits