...
parser.add_argument('dataset', type=str, choices=['ucf101', 'hmdb51','gesture', 'huda'])
parser.add_argument('split', type=int, choices=[1, 2, 3],
                    help='on which split to test the network')
parser.add_argument('modality', type=str, choices=['rgb', 'flow','depth','depth-flow'])
...
def eval_video(video):
    ...
    if args.modality == 'rgb':
        cnt_indexer = 1
    elif args.modality == 'flow' or args.modality == 'depth-flow':
        cnt_indexer = 2
    ...
    print 'Evaluating video {}.'.format(vid)
    frame_scores = []
    for tick in frame_ticks:
        if args.modality == 'rgb' or args.modality=='depth':
            ...
            try:
                ...
                sys.stderr.write('Exception in video {}.'.format(vid)+'\n')
                sys.stderr.write('frame name:', os.path.join(video_frame_path, name)+'\n')
                scores = np.zeros_like(frame_scores[0])

            frame_scores.append(scores)
        if args.modality == 'flow' or args.modality == 'depth-flow':
