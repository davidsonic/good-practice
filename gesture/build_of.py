...
# files=os.listdir('/data4/jiali/data/iso_test_depth')
...
def run_optical_flow(vid_item, dev_id=0):
    ...
    #     print '{} already done'.format(vid_name)
    #     return True


    out_full_path = os.path.join(out_path, vid_name)

    if not os.path.exists(out_full_path) or len(os.listdir(out_full_path))==0:
        ...
        cmd = os.path.join(df_path + 'build/extract_gpu')+' -f {} -x {} -y {} -i {} -b 20 -t 1 -d {} -s 1 -o {} -w {} -h {}'.format(
            ...
            dev_id, out_format, new_size[0], new_size[1])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--df_path", type=str, default='/data4/jiali/good-practice/lib/dense_flow/', help='path to the dense_flow toolbox')
    ...
    vid_list=[]
    with open('/data4/jiali/good-practice/gesture/valid_depth.txt','r') as f:
        file_all=f.readlines()
        num=len(file_all)
        for i in range(num):
            tmp=file_all[i].split(' ')[0]
            vid_list.append(work_root+tmp)
